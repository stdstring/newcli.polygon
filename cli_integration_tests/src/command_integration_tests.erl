...
integration_test_() ->
    integration_tests_common:create_tests_entry([
        ...
        {["login", "guest", "idclip", "pong", "logout"], [?LOGIN, ?GREETING, "guest@CliDemo>Command's execution is failed due to the following: {parser_fail,[112,111,110,103],unknown_command}", "Command execution failed. Return code is 255", ?GUEST_LOGOUT], "command: execute unknown command by user"},
        {["login", "root", "iddqd", "pong", "logout"], [?LOGIN, ?GREETING, "root@CliDemo#Command's execution is failed due to the following: {parser_fail,[112,111,110,103],unknown_command}", "Command execution failed. Return code is 255", ?ADMIN_LOGOUT], "command: execute unknown command by admin"},
