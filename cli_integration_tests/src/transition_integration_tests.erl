...
integration_test_() ->
    integration_tests_common:create_tests_entry([
        ...
        {["login", "root", "iddqd", "exit", "logout"], [?LOGIN, ?GREETING, "root@CliDemo#Command's execution is failed due to the following: {precondition_check_fail,unsuitable_command}", "Command execution failed. Return code is 255", ?ADMIN_LOGOUT], "transition: exit from fundamental mode"},
        ...
        {["login", "root", "iddqd", "end", "logout"], [?LOGIN, ?GREETING, "root@CliDemo#Command's execution is failed due to the following: {precondition_check_fail,unsuitable_command}", "Command execution failed. Return code is 255", ?ADMIN_LOGOUT], "transition: end fundamental mode"}]).