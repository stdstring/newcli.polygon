...
syntax_analyzer_process_test_() ->
    ...
    [{"parse 'ping 192.168.0.1'",
     ...
     {"parse 'p 192.168.0.1'",
      success_execution([?WORD_TOKEN("p"), ?WORD_TOKEN("192.168.0.1"), ?END_TOKEN], Config, ?PING_FUNCTION, ["192.168.0.1"])},
     {"parse 'interface \"some interface\"'",
      success_execution([?WORD_TOKEN("interface"), ?STRING_TOKEN("some interface"), ?END_TOKEN], Config, ?INTERFACE_FUNCTION, ["some interface"])},
     {"parse 'exit'",
      success_execution([?WORD_TOKEN("exit"), ?END_TOKEN], Config, ?EXIT_FUNCTION, [])},
