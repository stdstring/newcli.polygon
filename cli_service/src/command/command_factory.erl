...
-spec process(Source :: string(), LexConfig :: #lex_analyzer_config{}, SyntaxConfig :: #syntax_analyzer_config{}, EntryModuleName :: atom()) ->
    {'true', Fun :: fun((CliFsm :: pid(), ClientHandler :: pid(), Context :: [{atom(), term()}]) -> 'ok')} | {'false', Reason :: term()}.
process(Source, LexConfig, SyntaxConfig, EntryModuleName) ->
    case command_parser:process(Source, LexConfig, SyntaxConfig) of
        {true, #command{} = Command} ->
            case code_generator:generate(EntryModuleName, ?ENTRY_FUNC, Command, ?MODULE_DEFS) of
