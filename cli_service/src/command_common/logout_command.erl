...
execute(Args, Stdout, Stderr, ExecContext) ->
    case check_args(Args) of
        false -> command_utils:process_error(Stderr, ?BAD_ARGS_MESSAGE, ?BAD_ARGS_RETURN_CODE, ExecContext);
...
process_command(Stdout, Stderr, ExecContext) ->
    case lists:keyfind(?USER_KEY, 1, ExecContext) of
        false -> command_utils:process_error(Stderr, ?MISSING_USER_MESSAGE, ?MISSING_USER_RETURN_CODE, ExecContext);
        {?USER_KEY, User} ->
            ...
            Message = string_utils:format(?LOGOUT_INFO_TEMPLATE, [User#user.username]),
