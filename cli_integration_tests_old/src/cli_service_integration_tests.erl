...
-spec is_backend_command_exist(CommandsInfo :: {'command_info', Name :: atom(), Body :: [string()], Help :: string()}, ExpectedCommand :: string()) -> boolean().
...
-spec is_frontend_command_exist(Commands :: [string()], ExpectedCommand :: string()) -> boolean().
is_frontend_command_exist(Commands, ExpectedCommand) ->
    lists:any(fun(Command) -> Command == ExpectedCommand end, Commands).

-spec backend_interact() -> {'command_info', Name :: atom(), Body :: [string()], Help :: string()}.
backend_interact() ->
    %% TODO (std_string) : try use include files from cli_backend
    {commands_info_result, CommandsInfo} = gen_server:call({?BACKEND_PROCESS, ?BACKEND_NODE}, ?BACKEND_COMMAND),
    CommandsInfo.

-spec frontend_interact() -> {OutputResponse :: {'command_out', CommandOut :: string()}, EndResponse :: {'end', Prompt :: string()}}.
frontend_interact() ->
